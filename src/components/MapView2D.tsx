...
export default function MapView2D({
  ...
  const [explosionRays, setExplosionRays] = useState<Array<{
    angle: number;
    id: number;
  }>>([]);
  ...
  const threatStats = useMemo(() => {
    ...
    visibleAircraft.forEach(aircraft => {
      ...
      if (aircraft.barometricAltitude !== null) {
        ...
        altitudeCount++;
        if (aircraft.barometricAltitude < 3000) stats.altitudeRanges.low++;else if (aircraft.barometricAltitude < 10000) stats.altitudeRanges.medium++;else stats.altitudeRanges.high++;
      }
      ...
      if (aircraft.velocity !== null) {
        ...
        speedCount++;
        if (aircraft.velocity < 100) stats.speedRanges.slow++;else if (aircraft.velocity < 250) stats.speedRanges.medium++;else stats.speedRanges.fast++;
      }
    });
  ...
  // Load country data
  useEffect(() => {
    fetch('/countries.json').then(response => response.json()).then((topology: any) => {
    ...
    try {
      ...
      fetch(apiUrl).then(response => response.json()).then((data: {
        ...
        if (data.success && data.data) {
          ...
          const aircraftWithScores = data.data.filter(a => a.latitude !== null && a.longitude !== null).map(aircraft => {
  ...
  useEffect(() => {
    if (sensitivity[0] >= 0.03) {
      const errorInterval = setInterval(() => {
        const fakeErrors = ["CRITICAL: RADIATION DETECTED", "SYSTEM FAILURE: DEFENSE GRID OFFLINE", "WARNING: MISSILE LAUNCH DETECTED", "ERROR: COMMUNICATION LOST", "ALERT: PERIMETER BREACH", "CRITICAL: REACTOR OVERLOAD", "WARNING: HOSTILE AIRCRAFT DETECTED", "ERROR: COMMAND CENTER COMPROMISED", "ALERT: NUCLEAR SIGNATURE DETECTED", "CRITICAL: CIVILIAN CASUALTIES REPORTED"];
  ...
  useEffect(() => {
    ...
    const availableCountries = countries.map(c => c.properties.name).filter(name => name !== selectedCountry);
    ...
    if (sensitivityValue >= 0.02) {
      ...
      for (let i = 0; i < numRandomConflicts && i < 150; i++) {
  ...
  const renderCountry = (country: GeoJSONFeature, width: number, height: number) => {
    ...
    return allPaths.filter(path => path !== '').map((path, index) => <path key={`${countryName}-${index}`} d={path} fill="none" stroke="#ff3333" strokeWidth={isSelected ? 2.5 : 1.2} strokeOpacity={isSelected ? 1 : 0.6} className={isSelected ? "animate-pulse" : ""} style={isSelected ? {
  ...
  return <div className="relative w-full h-screen bg-background overflow-hidden">
      {/* Timer display */}
      {timerStarted && <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-30">
          <div className="font-mono text-6xl font-bold tracking-wider animate-pulse" style={{
        color: getTimerColor(),
      ...
      <div className="absolute top-4 left-4 space-y-2 z-20">
        {errorMessages.map(error => <div key={error.id} className="bg-red-900/90 border border-red-500 px-4 py-2 text-red-100 font-mono text-xs tracking-wider animate-fade-in" style={{
      ...
      <div className="absolute inset-0 pointer-events-none opacity-5 crt-effect" style={{
      backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(255, 0, 0, 0.15) 2px, rgba(255, 0, 0, 0.15) 4px)'
    }} />

      {/* Explosion effect */}
      {isExploding && <div className="absolute inset-0 z-50 pointer-events-none">
        ...
        boxShadow: '0 0 200px 100px rgba(255, 100, 0, 0.8)'
          ...
          {/* Shockwave rings */}
          {[0, 0.3, 0.6, 0.9].map((delay, i) => <div key={i} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 rounded-full border-4 border-orange-500" style={{
        ...
        opacity: 0
          ...
          {Array.from({
        length: 50
      }).map((_, i) => <div key={i} className="absolute top-1/2 left-1/2 w-2 h-2 bg-red-600 rounded" style={{
      ...
      {isLoading && <div className="absolute inset-0 flex items-center justify-center z-10 bg-background">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-8 animate-fade-in">
        <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} className="w-full h-full" style={{
        maxHeight: '100%',
        maxWidth: '100%',
        animation: sensitivity[0] >= 0.03 ? `mapShake ${Math.max(0.2, 1 - visibleConflictCount / 200)}s infinite` : 'none',
        ...
        transition: 'all 2s ease-out'
          ...
          return <g key={`conflict-${enemyCountry}-${idx}`} className="animate-fade-in" style={{
            animationDelay: `${idx * 100}ms`
          }}>
                {/* Level 1-2: Warning lines */}
                {warLevel >= 1 && <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={warLevel === 1 ? "#ffaa00" : "#ff6600"} strokeWidth={warLevel === 1 ? "0.5" : "1"} strokeDasharray={warLevel === 1 ? "5,5" : "3,3"} opacity="0">
                    <animate attributeName="opacity" from="0" to={warLevel === 1 ? "0.4" : "0.6"} dur="0.5s" fill="freeze" />
                ...
                {warLevel >= 2 && <>
                    <defs>
                      <path id={pathId} d={`M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`} fill="none" />
                    </defs>
                    
                    {/* Missile trail */}
                    <path d={`M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`} fill="none" stroke={warLevel >= 4 ? "#ff0000" : "#ff8800"} strokeWidth={warLevel >= 3 ? "2" : "1.5"} opacity="0" style={{
                filter: `drop-shadow(0 0 ${warLevel * 2}px ${warLevel >= 4 ? "#ff0000" : "#ff8800"})`
              }}>
                      <animate attributeName="opacity" from="0" to="0.7" dur="0.5s" fill="freeze" />
                    </path>
                    
                    {/* Animated missile */}
                    <circle r={warLevel >= 4 ? "4" : "3"} fill={warLevel >= 4 ? "#ff0000" : "#ffff00"} style={{
                filter: `drop-shadow(0 0 ${warLevel * 3}px ${warLevel >= 4 ? "#ff0000" : "#ffff00"})`
              }}>
                      <animateMotion dur={`${missileDuration}s`} repeatCount="indefinite" begin={missileDelay}>
                        <mpath href={`#${pathId}`} />
                      </animateMotion>
                    </circle>
                  </>}
                ...
                {warLevel >= 3 && <g>
                    ...
                    <circle cx={x2} cy={y2} r="0" fill="none" stroke={warLevel >= 5 ? "#ff0000" : "#ff6600"} strokeWidth="2" opacity="0">
                      <animate attributeName="r" from="5" to="25" dur="1.5s" begin={`${explosionStart}s`} repeatCount="indefinite" />
                      <animate attributeName="opacity" from="0.7" to="0" dur="1.5s" begin={`${explosionStart}s`} repeatCount="indefinite" />
                    </circle>
                  </g>}
                ...
                {sensitivity[0] >= 0.028 && <>
                    <defs>
                      <path id={`counter-${pathId}`} d={`M ${x2} ${y2} Q ${cx} ${cy} ${x1} ${y1}`} fill="none" />
                    </defs>
                    
                    {/* Counter missile trail */}
                    <path d={`M ${x2} ${y2} Q ${cx} ${cy} ${x1} ${y1}`} fill="none" stroke="#ff0000" strokeWidth="2" opacity="0.6" style={{
                filter: "drop-shadow(0 0 10px #ff0000)"
              }} />
                    
                    {/* Counter missile */}
                    <circle r="4" fill="#ff0000" style={{
                filter: "drop-shadow(0 0 15px #ff0000)"
              }}>
                      <animateMotion dur="2.5s" repeatCount="indefinite" begin={`${parseFloat(missileDelay) + 1.5}s`}>
                        <mpath href={`#counter-${pathId}`} />
                      </animateMotion>
                    </circle>
                    
                    {/* Home country explosion */}
                    <circle cx={x1} cy={y1} r="10" fill="#ff0000" opacity="0" style={{
                filter: "blur(5px)"
              }}>
                      <animate attributeName="opacity" values="0;1;0" dur="0.3s" begin={`${parseFloat(missileDelay) + 4}s`} repeatCount="indefinite" />
                    </circle>
                  </>}
                
                {/* Enemy country highlight - simplified */}
                <circle cx={x2} cy={y2} r="4" fill="#ff0000" opacity={warLevel >= 3 ? 0.6 : 0.4} />
              </g>;
        })}
          ...
          const cx = x1 + dx / 2 + dy * 0.3;
          const cy = y1 + dy / 2 - dx * 0.3;
          return <g key={`chain-conflict-${idx}`} className="animate-fade-in">
                {/* Chain reaction missile path */}
                <path d={`M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`} fill="none" stroke="#ffaa00" strokeWidth="1.2" opacity="0" style={{
              filter: "drop-shadow(0 0 4px #ffaa00)"
                ...
                <circle r="2.5" fill="#ffff00" style={{
              filter: "drop-shadow(0 0 5px #ffff00)"
            }}>
                  <animateMotion dur="3.5s" repeatCount="indefinite" begin={`${idx * 0.15}s`}>
                ...
                <defs>
                  <path id={`chain-path-${idx}`} d={`M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`} fill="none" />
                </defs>
              </g>;
        })}
          ...
          return <g key={`random-conflict-${idx}`} className="animate-fade-in">
                {/* Curved missile path for random conflicts */}
                <path d={`M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`} fill="none" stroke="#ff6600" strokeWidth="1" opacity="0" style={{
              filter: "drop-shadow(0 0 3px #ff6600)"
                ...
                <circle r="2" fill="#ffaa00" style={{
              filter: "drop-shadow(0 0 4px #ffaa00)"
            }}>
                  <animateMotion dur="4s" repeatCount="indefinite" begin={`${idx * 0.1}s`}>
                ...
                {/* Path definition for motion */}
                <defs>
                  <path id={`random-path-${idx}`} d={`M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`} fill="none" />
                </defs>
              </g>;
        })}
          
          {/* Spinning radar on selected country - Below planes but above everything else */}
          {selectedCountry && countryCentroids[selectedCountry] && !isExploding && <g>
              {(() => {
            ...
            return <>
                    ...
                    <circle cx={cx} cy={cy} r={radarRadius} fill={`url(#${uniqueId}-bg)`} />
                    
                    {/* Radar sweep circles (concentric rings) */}
                    <circle cx={cx} cy={cy} r={radarRadius} fill="none" stroke="#00ff00" strokeWidth="1.5" opacity="0.8" />
                    <circle cx={cx} cy={cy} r={radarRadius * 0.66} fill="none" stroke="#00ff00" strokeWidth="1.2" opacity="0.6" />
                    <circle cx={cx} cy={cy} r={radarRadius * 0.33} fill="none" stroke="#00ff00" strokeWidth="0.9" opacity="0.6" />
                    
                    {/* Cross-hair lines (north/south, east/west) */}
                    <line x1={cx} y1={cy - radarRadius} x2={cx} y2={cy + radarRadius} stroke="#00ff00" strokeWidth="0.8" opacity="0.4" />
                    <line x1={cx - radarRadius} y1={cy} x2={cx + radarRadius} y2={cy} stroke="#00ff00" strokeWidth="0.8" opacity="0.4" />
                      ...
                      <g clipPath={`url(#${uniqueId}-clip)`}>
                        <g transform={`translate(${cx}, ${cy})`}>
                          <g>
                            ...
                            <path d={`M 0,0 L ${radarRadius},0 A ${radarRadius},${radarRadius} 0 0,1 ${radarRadius * Math.cos(Math.PI / 8)},${-radarRadius * Math.sin(Math.PI / 8)} Z`} fill={`url(#${uniqueId}-sweep-cone)`} opacity="0.6" />
                            {/* Main sweep line */}
                            <line x1={0} y1={0} x2={radarRadius} y2={0} stroke={`url(#${uniqueId}-sweep)`} strokeWidth="3" strokeLinecap="round" />
                          </g>
                        </g>
                      </g>
                    </g>
                    ...
                    {/* Center dot */}
                    <circle cx={cx} cy={cy} r="3" fill="#00ff00" opacity="0.9">
                      <animate attributeName="opacity" values="0.9;0.5;0.9" dur="2s" repeatCount="indefinite" />
                    </circle>
                    
                    {/* Outer pulse ring */}
                    <circle cx={cx} cy={cy} r={radarRadius} fill="none" stroke="#00ff00" strokeWidth="2" opacity="0">
                      <animate attributeName="opacity" values="0;0.6;0" dur="2s" repeatCount="indefinite" />
                      <animate attributeName="r" values={`${radarRadius};${radarRadius * 1.1}`} dur="2s" repeatCount="indefinite" />
                    </circle>
                  </>;
          })()}
            </g>}
          ...
          if (selectedCountryCentroid) {
            const bearing = calculateBearing(plane.latitude, plane.longitude, selectedCountryCentroid[1],
            // lat
          ...
          return <g key={plane.icao24} transform={`translate(${x}, ${y})`} style={{
            cursor: 'pointer'
          }} onClick={() => setSelectedAircraft(plane)} className="aircraft-icon">
                ...
                <g transform={`rotate(${rotation})`}>
                  <g>
                    <animateTransform attributeName="transform" type="translate" values={`0,0;${moveX},${moveY};0,0`} dur="3s" repeatCount="indefinite" />
                    {/* SVG plane icon pointing right */}
                    <svg x="-8" y="-8" width="16" height="16" viewBox="0 0 24 24" fill="none">
                      <path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z" fill={planeColor} stroke="#ffffff" strokeWidth="1" />
                    </svg>
                  </g>
                </g>

                {/* Threat level indicator above plane */}
                <text x="0" y="-18" fontSize="6" fill={planeColor} textAnchor="middle" fontFamily="monospace" fontWeight="bold" opacity="0.8">
      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button variant="ghost" size="icon" className="absolute bottom-4 right-4 bg-card/95 border-2 border-primary hover:bg-primary/20 hover:border-war-glow transition-all z-20 animate-scale-in">
      ...
      <Dialog open={showThreatsDashboard} onOpenChange={open => {
      ...
      if (!open && isExploding && onGameOver) {
        ...
        <DialogContent className="bg-card/95 border-2 border-primary max-w-5xl backdrop-blur-sm max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 mt-4">
            ...
            {conflictHistory.length > 1 && <div className="bg-background/50 border border-primary/30 p-4">
                ...
                <div className="relative h-48 border border-primary/20">
                  ...
                  <div className="absolute top-2 right-2 flex gap-4 text-[10px]">
                    ...
                    <div className="flex items-center gap-1">
                      <div className="w-3 h-0.5 bg-orange-400" style={{
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <div className="w-full bg-background h-6 relative overflow-hidden">
                    <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-orange-600 to-orange-400 flex items-center justify-center" style={{
                    width: `${enemyCountries.length + chainConflicts.length + randomConflicts.length > 0 ? enemyCountries.length / (enemyCountries.length + chainConflicts.length + randomConflicts.length) * 100 : 0}%`
                  }}>
                ...
                <div>
                  ...
                  <div className="w-full bg-background h-6 relative overflow-hidden">
                    <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-yellow-600 to-yellow-400 flex items-center justify-center" style={{
                    width: `${enemyCountries.length + chainConflicts.length + randomConflicts.length > 0 ? chainConflicts.length / (enemyCountries.length + chainConflicts.length + randomConflicts.length) * 100 : 0}%`
                  }}>
                      <span className="text-[10px] font-bold text-white drop-shadow">
                ...
                <div>
                  ...
                  <div className="w-full bg-background h-6 relative overflow-hidden">
                    <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-red-600 to-red-400 flex items-center justify-center" style={{
                    width: `${enemyCountries.length + chainConflicts.length + randomConflicts.length > 0 ? randomConflicts.length / (enemyCountries.length + chainConflicts.length + randomConflicts.length) * 100 : 0}%`
                  }}>
                      <span className="text-[10px] font-bold text-white drop-shadow">
                ...
                <div className="flex items-center justify-center h-32">
                  <div className="relative w-32 h-32">
                    <svg className="w-full h-full -rotate-90">
                      <circle cx="64" cy="64" r="56" fill="none" stroke="rgba(255,51,51,0.2)" strokeWidth="8" />
                      <circle cx="64" cy="64" r="56" fill="none" stroke="#ff3333" strokeWidth="8" strokeDasharray={`${2 * Math.PI * 56}`} strokeDashoffset={`${2 * Math.PI * 56 * (1 - sensitivity[0] * 10)}`} strokeLinecap="round" />
                    </svg>
                ...
                <div className="space-y-3 mt-4">
                  <div>
                    ...
                    <div className="w-full bg-background h-2">
                      <div className={`h-full ${sensitivity[0] >= 0.03 ? "bg-red-500" : sensitivity[0] >= 0.025 ? "bg-yellow-500" : "bg-green-500"}`} style={{
                      width: `${sensitivity[0] * 1000}%`
                    }} />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="w-full bg-background h-2">
                      <div className="h-full bg-gradient-to-r from-orange-600 to-red-600" style={{
                      width: `${enemyCountries.length / Math.max(countries.length - 1, 1) * 100}%`
                    }} />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="w-full bg-background h-2">
                      <div className="h-full bg-red-600" style={{
                      width: `${randomConflicts.length / Math.max(enemyCountries.length + chainConflicts.length + randomConflicts.length, 1) * 100}%`
                    }} />
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-background/50 border border-primary/30 p-4">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {Object.entries(threatStats.byCountry).sort(([, a], [, b]) => b - a).slice(0, 10).map(([country, count]) => <div key={country}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-muted-foreground">{country}</span>
                        <span className="text-orange-400">{count}</span>
                      </div>
                      <div className="w-full bg-background h-3 relative overflow-hidden">
                        <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-orange-600 to-orange-400" style={{
                    width: `${threatStats.total > 0 ? count / threatStats.total * 100 : 0}%`
                  }} />
                      </div>
                    </div>)}
              </div>
      ...
      <Dialog open={selectedAircraft !== null} onOpenChange={open => !open && setSelectedAircraft(null)}>
        <DialogContent className="bg-card/95 border-2 border-primary max-w-md backdrop-blur-sm">
          ...
          {selectedAircraft && <div className="space-y-3 text-sm">
              {/* THREAT LEVEL DISPLAY */}
              {selectedAircraft.threatLevel && <div className="bg-red-950/30 border-2 border-red-500/50 p-4 rounded">
                ...
                color: selectedAircraft.threatLevel > 0.027 ? '#ff0000' : selectedAircraft.threatLevel > 0.025 ? '#ff8800' : selectedAircraft.threatLevel > 0.023 ? '#ffff00' : '#00ff00'
              }}>
                ...
                backgroundColor: selectedAircraft.threatLevel > 0.027 ? '#ff0000' : selectedAircraft.threatLevel > 0.025 ? '#ff8800' : selectedAircraft.threatLevel > 0.023 ? '#ffff00' : '#00ff00'
              }} />
                  </div>
                  <p className="text-[10px] text-muted-foreground mt-2 italic">
                    {selectedAircraft.threatLevel > 0.027 ? "EXTREME THREAT - Immediate response recommended" : selectedAircraft.threatLevel > 0.025 ? "HIGH THREAT - Close monitoring required" : selectedAircraft.threatLevel > 0.023 ? "MODERATE THREAT - Standard protocols" : "LOW THREAT - Routine surveillance"}
                  </p>
                </div>}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-primary font-mono">
                    {selectedAircraft.verticalRate ? `${selectedAircraft.verticalRate > 0 ? '+' : ''}${Math.round(selectedAircraft.verticalRate)} m/s` : 'N/A'}
                  </p>
                </div>
                <div>
              ...
              {selectedAircraft.lastContact && <div className="pt-2 border-t border-primary/20">
      ...
      {isAircraftLoading && <div className="absolute bottom-20 right-8 bg-card/95 border-2 border-primary px-4 py-2 z-20 animate-scale-in">
      ...
      {visibleConflictCount > 50 && <div className="absolute inset-0 pointer-events-none" style={{
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 animate-scale-in bg-card/95 border-2 border-primary px-8 py-6 rounded-lg">
        <div className="space-y-4">
          ...
          <div className="w-64">
            <Slider value={sensitivity} onValueChange={setSensitivity} max={0.1} step={0.001} className="cursor-pointer" />
          </div>
          <div className="flex justify-between items-center text-xs gap-4">
            ...
            <button onClick={() => setShowThreatsDashboard(true)} className="text-primary font-mono text-[10px] hover:text-primary/80 hover:underline cursor-pointer transition-colors" disabled={enemyCountries.length + chainConflicts.length + randomConflicts.length === 0}>
